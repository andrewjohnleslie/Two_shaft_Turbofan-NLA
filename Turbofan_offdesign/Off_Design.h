//
// Created by Andrew on 25/07/2023.
//

#ifndef TURBOFAN_OFFDESIGN_OFF_DESIGN_H
#define TURBOFAN_OFFDESIGN_OFF_DESIGN_H
...
class Off_Design : public Design{
    ...
    //double h_ft_OD = 35000;                   // h in ft
    //double h_OD = h_ft_OD * 0.3048;              // CONVERTS ALTITUDE TO M
    double h_OD = 12000;                    // h in m


    //double Ta_OD = T_std - (0.0065 * h_OD);
    //double t_OD = (6356577 * h_OD)/(6356577 + h_OD);          //geopotential height
...
public:
    ...
    void myOffDesignFunction() {
        do {
            ...
            double P045_OD = P04_OD * pow((T045_OD/T04_OD), (gamma_e/(e_t * (gamma_e - 1))));           // Issue starts here
            double D_HP_ODa = D_HP_D * sqrt(T_04) * P04_OD/(P_04 * sqrt(T04_OD));
            double B_OD = phi1 * D_BypN_OD/D_HP_ODa;                                                          // Bypass ratio - OFF-DESIGN
            double T05_OD = T045_OD * (1 - ((cp/cpe) * (T02_OD/T045_OD) * (B_OD + kt) * (pow(pif_OD, (gamma - 1)/(e_c * gamma)) - 1)));
            ...
            if (y_core_OD <= Pcrit_core){                   // If P05/Pa < P05/Pcrit (Core Nozzle PR < Critical PR)
                ...
                *m9OD = (gamma_e/(gamma_e - 1)) * sqrt(2 * (pow((P05_OD/P9_OD), (-2/gamma_e)) - pow((P05_OD/P9_OD), (-(gamma_e + 1)/gamma_e))));     // Unchoked
            }else{
                std::cout << std::endl;
            ...
            if (y_bypass_OD <= Pcrit_bypass){                   // If P05/Pa < P05/Pcrit (Core Nozzle PR < Critical PR)
                ...
                *m19OD = (gamma/(gamma - 1)) * sqrt(2 * (pow((P019_OD/P19_OD), (-2/gamma)) - pow((P019_OD/P19_OD), (-(gamma + 1)/gamma))));      // Unchoked
            }else{
                std::cout << std::endl;
                std::cout << "OFF DESIGN: Bypass nozzle is choked! \n " << std::endl;
                double *m19OD = &m19_bar_OD;
                *m19OD = (gamma/(sqrt(gamma - 1))) * (pow(0.5 * (gamma + 1), ((-1 * (gamma + 1))/(2 * (gamma - 1)))));         // Choked
            }
            ...
            *error = 100 * (D_HP_ODa - D_HP_ODb)/D_HP_ODa;
            ...
            double m2_bar_OD = (sqrt(cp * T02_OD) / P02_OD) * ((m4_bar * (1/D_HP_ODa) * m_core_rat * P03_OD/(sqrt(cpe * T04_OD))) + (m19_bar_OD * (1/D_BypN_OD) * m_byp_rat * P013_OD/sqrt(cp * T013_OD)));
            double m23_bar_OD = (sqrt(cp * T023_OD) * m4_bar * (1/D_HP_ODa) * m_core_rat * P03_OD)/(sqrt(cpe * T04_OD) * P023_OD);
            ...
            double eta_p_OD = (Fs_OD * Vf_OD)/(Fs_OD * Vf_OD + 0.5 * (1/(1 + B_OD)) * pow((V9_OD-Vf_OD), 2) + 0.5 * (B_OD/(1 + B_OD)) * pow((V19_OD - Vf_OD), 2));
            double eta_t_OD = (Fs_OD * Vf_OD + 0.5 * (1/(1 + B_OD)) * pow((V9_OD - Vf_OD), 2) + 0.5 * (B_OD/(1 + B_OD)) * pow((V19_OD - Vf_OD), 2))/((f_OD * Qr)/(1 + B_OD));
            double eta_o_OD = (Fs_OD * Vf_OD)/((f_OD * Qr)/(1 + B_OD));

            double Fn_rat = (D_CoreN_OD/D_CoreN_D) * ((B_OD * (V19_OD - Vf_OD) + (V9_OD - Vf_OD)))/(B_D * (V19 - V_f) + (V9 - V_f));
            ...
            */
            ...
            if (std::abs(ERR) < 1e-6) {
                ...
                if (outputFile.is_open()) {
                    ...
                    /*outputFile << Mf_OD << "," << F_n << "," << std::setprecision(10) << SFC_OD << "," << Fs_OD << std::setprecision(8) << "," << f_OD << "," << B_OD << ","
                               << pif_OD << "," << eta_p_OD << "," << eta_t_OD << "," << eta_o_OD << "," << piOPR_OD << ","
                               << piLPC_OD << "," << piHPC_OD << "," << piLPT_OD << "," << Tratio_OD << ","
                               << m2_bar_OD << "," << m23_bar_OD << "," << SFC_rat << "," << Fs_rat << ","
                               << m_core_rat << "," << m_byp_rat << "," << m2cor_OD_ND << "," << m23cor_OD_ND << ","
                               << Fg_rat << "," << Fn_rat  << "," << mdot9_OD << "," << mdot19_OD << ","
                               << V9_OD << "," << V19_OD  << "," << T045_OD << "," << T05_OD << std::endl;
                               */

                    outputFile << Mf_OD << "," << Vf_OD << "," << std::setprecision(10) << Ta_OD << "," << Pa_OD << "," << T02_OD << "," << P02_OD << ","
                               << T023_OD << "," << P023_OD << "," << T03_OD << "," << P03_OD
                               << "," << piLPC_OD << "," << piHPC_OD << "," << piOPR_OD << "," << T04_OD << "," << P04_OD << ","
                               << Tratio_OD << "," << T045_OD << "," << P045_OD << ","
                               << T05_OD << "," << P05_OD << "," << piLPT_OD << "," << piHPT_OD << "," << T09_OD << ","
...
#endif //TURBOFAN_OFFDESIGN_OFF_DESIGN_H
